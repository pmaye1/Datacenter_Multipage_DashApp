...
import pathlib
dash.register_page(__name__,path='/HET_CT_FTRAB')
...
controlfemaleFemur_df=pd.read_csv(DATA_PATH.joinpath('controlfemale_femur.csv'))
controlfemaleFemur_df=controlfemaleFemur_df.dropna()

controlmaleFemur_df=pd.read_csv(DATA_PATH.joinpath('controlmale_femur.csv'))
...
data1= pd.read_csv(DATA_PATH.joinpath('female_Femur_average.csv'))
data2= pd.read_csv(DATA_PATH.joinpath('male_Femur_average.csv'))


ratio_df=pd.DataFrame(columns=['Gene Symbol', 'Female Femur', 'Male Femur'])

#calculating the average BV/TV of control groups
FemaleFemur_Count=controlfemaleFemur_df['BV/TV'].count()
FF_BVTVave=(sum(controlfemaleFemur_df['BV/TV'])/FemaleFemur_Count)*100
MaleFemur_Count=controlmaleFemur_df['BV/TV'].count()
MF_BVTVave=(sum(controlmaleFemur_df['BV/TV'])/MaleFemur_Count)*100


ratio_df['Gene Symbol']=data1['gene_symbol']
ratio_df['Female Femur']=((((data1['BV/TV'])*100/FF_BVTVave)-1)*100)
ratio_df['Male Femur']=((((data2['BV/TV'])*100/MF_BVTVave)-1)*100)
...
dbc.Col([
    dbc.Row([scroll_layout()],style={'marginLeft':90}),
    # row  2
    dbc.Row(
        [html.H1('Femur µCT Outcomes for Male & Female Heterozygous Mice',
                        style={'color': 'black', 'fontSize': 25,'fontFamily': "Verdana","font-weight":"600",
                               'margin':'auto'}),

        ], style={'height': 60, 'padding-bottom': 0, 'padding-top': 0, 'margin-bottom':0, 'margin-left':175,'margin-right':-200},
    ...
    dbc.Row([
        ...
        {
            'if': {
                'filter_query': '{Female Femur} <= -19.0',
                'column_id': 'Female Femur'},
            'backgroundColor': 'dodgerblue',
            'color': 'white',
         },
            {'if': {
                'filter_query': '{Male Femur} <= -18.5',
                'column_id': 'Male Femur'},
            'backgroundColor': 'dodgerblue',
            'color': 'white'
            },
        ...
        # dbc.Col([], width=1),
        dbc.Col([
            dcc.Graph(id='box_plot15', config={'modeBarButtonsToRemove': ['pan2d', 'displaylogo','autoScale2d','resetScale2d','hoverClosestCartesian',
    ...
    dbc.Row([
        ...
        dbc.Col([html.P('P values were calculated based on the outcomes of a two sample t-test',
                        style={'color': 'black', 'fontSize': 12,'fontFamily': "Verdana",
                               'margin-left':50,'margin-right':-30, 'textAlign':'center','margin-top':0}),
        ...
        dbc.Col([
            html.Div([
                dbc.Button(
                    ...
                    style=CT,
                    n_clicks=0,
                    href='https://ucsci.uchc.edu/bonebase/GeneEntry.html?GeneSymbol=Alg3+MicroCT_FemurTrab',
                    target='_blank',
                    external_link=True
                ),
            ],style={'margin-left':10}),
        ], width=2, style={'margin-left':100, 'margin-top':0}),
        dbc.Col(html.Div([layout_SB()], style={'margin-left': -10}),
                width=2, style={'margin-left': -80, 'margin-top': 0, 'margin-right': -30}),
...
@callback(
    Output(component_id='box_plot15', component_property='figure'),
    Input(component_id='Table10', component_property='selected_rows'),
    )
...
def update_boxplots(selected_rows):
        if len(selected_rows) == 0:
            ...
            bvtv = df_filtered['BV/TV'] * 100
            bvtv2 = df_filtered_MF['BV/TV'] * 100
            ...
            y = bvtv
            y1 = controlfemaleFemur_df['BV/TV'] * 100
            y2 = bvtv2
            y3 = controlmaleFemur_df['BV/TV'] * 100
            y4 = TbN
        ...
        box_plot.update_yaxes(title_text="<b>Female</b>", row=1, col=1, range=[0, 25],gridcolor='#D3D3D3', title_font=dict(size=18,family='Arial'),tickfont=dict(size=14))
        box_plot.update_yaxes(row=2, col=1,title_text="<b>Male</b>", range=[0, 25],gridcolor='#D3D3D3',title_font=dict(size=18,family='Arial'),tickfont=dict(size=14))
...
@callback(
    Output('btn-nclicks-10', 'href'),
    Input('btn-nclicks-10','n_clicks'),
    Input('Table10','selected_rows')
)
def display_page_contents(n_clicks,selected_rows):
    ...
    if len(selected_rows) > 0 & n_clicks >= 0:
        ...
        s = value
        n = ''.join([str(elem) for elem in s])
        string = 'https://ucsci.uchc.edu/bonebase/GeneEntry.html?GeneSymbol=Alg3+MicroCT_FemurTrab'
        pathname = string.replace('Alg3', n)
        print(pathname)
        return (pathname)
    elif n_clicks==1 & len(selected_rows)== 0:
        return 'https://ucsci.uchc.edu/bonebase/GeneEntry.html?GeneSymbol=Alg3+MicroCT_FemurTrab'
