...
dash.register_page(__name__,path='/HET_CT_VTRAB')
...
controlfemaleVertebra_df=pd.read_csv(DATA_PATH.joinpath('controlfemale_Vertebra.csv'))
controlfemaleVertebra_df=controlfemaleVertebra_df.dropna()

controlmaleVertebra_df=pd.read_csv(DATA_PATH.joinpath('controlmale_Vertebra.csv'))
...
def layout():
   ...
   # row  2
    dbc.Row(
        [html.H1('Vertebra µCT Outcomes for Male & Female Global Heterozygous Mice',
                        style={'color': 'black', 'fontSize': 25,'fontFamily': "Verdana","font-weight":"600",
                               'margin':'auto'}),

        ],  align="center", style={'height': 60, 'padding-bottom': 0, 'padding-top': 0, 'margin-bottom':0, 'margin-left':20, 'margin-right':-200},
    ...
    dbc.Row([
        dbc.Col([
            ...
            {'if': {
                'filter_query': '{Female Vertebra} >= 9.5',
        ...
        {
            'if': {
                'filter_query': '{Female Vertebra} <= -8.0',
                'column_id': 'Female Vertebra'},
            'backgroundColor': 'dodgerblue',
            'color': 'white',
         },
        ...
        # dbc.Col([], width=1),
        dbc.Col([
            dcc.Graph(id='box_plot18', config={'modeBarButtonsToRemove': ['pan2d', 'displaylogo','autoScale2d','resetScale2d','hoverClosestCartesian',
     ...
     ], style={'height': 650, 'padding-bottom': 0, 'padding-top': 0, 'margin-left':-75,'margin-bottom':0,'justify':'center'}),
        ...
        dbc.Col([html.P('Numbers represent the % increase or decrease in bone volume per tissue volume'
                ' based on calculating ((BV/TV mutant/BV/TV control)-1)x100',
                        style={'color': 'black', 'fontSize': 12,'fontFamily': "Verdana",
                               'margin-top':0, 'margin-left':-70, 'margin-right':5}),
                ], width=4),
        ...
        dbc.Col([
            html.Div([
                dbc.Button(
                    ...
                    style=CT,
                    n_clicks=0,
                    href='https://ucsci.uchc.edu/bonebase/GeneEntry.html?GeneSymbol=Alg3+MicroCT_VertTrab',
                    target='_blank',
                    external_link=True
                ),
                ],style={'marginLeft':-10}),
        ], width=2, style={'margin-right': -20, 'margin-left': 70, 'margin-top': 0}),
...
@callback(
    Output(component_id='box_plot18', component_property='figure'),
    Input(component_id='Table12', component_property='selected_rows')
    )
...
def update_boxplots(selected_rows):
        if len(selected_rows) == 0:
            ...
            bvtv = df_filtered_FV['BV/TV']*100
            bvtv2 = df_filtered_MV['BV/TV']*100
            ...
            y = bvtv
            y1 = controlfemaleVertebra_df['BV/TV']*100
            y2 = bvtv2
            y3 = controlmaleVertebra_df['BV/TV']*100
            y4 = TbN
...
@callback(
    Output('btn-nclicks-12', 'href'),
    Input('btn-nclicks-12','n_clicks'),
    Input('Table12','selected_rows')
)
def display_page_contents(n_clicks,selected_rows):
    ...
    if len(selected_rows) > 0 & n_clicks >= 0:
        ...
        s = value
        n = ''.join([str(elem) for elem in s])
        string = 'https://ucsci.uchc.edu/bonebase/GeneEntry.html?GeneSymbol=Alg3+MicroCT_VertTrab'
        pathname = string.replace('Alg3', n)
        print(pathname)
        return (pathname)
    elif n_clicks==1 & len(selected_rows)== 0:
        return 'https://ucsci.uchc.edu/bonebase/GeneEntry.html?GeneSymbol=Alg3+MicroCT_VertTrab'
