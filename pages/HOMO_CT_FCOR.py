...
dash.register_page(__name__, path='/HOMO_CT_FCOR', name='femur cortical bone')
...
#Control Femur Data
controlfemaleFemur_df=pd.read_csv(DATA_PATH.joinpath('controlfemale_CortIM.csv'))
# controlfemaleFemur_df=controlfemaleFemur_df.dropna()
controlmaleFemur_df=pd.read_csv(DATA_PATH.joinpath('controlmale_CorticalIM.csv'))
...
data1= pd.read_csv(DATA_PATH.joinpath('female_Cortical_average.csv'))
data2= pd.read_csv(DATA_PATH.joinpath('male_Cortical_average.csv'))
...
def layout ():
    ...
    # row  2
    dbc.Row(
        [html.H1('Femur Cortical Bone µCT Outcomes for Global Homozygous KO Mice',
                        style={'color': 'black', 'fontSize': 25,'fontFamily': "Verdana","font-weight":"600",
                               'margin':'auto','text-align':'center'}),

        ], style={'height': 60, 'padding-bottom': 0, 'padding-top': 0, 'margin-bottom':0, 'margin-left':90, 'margin-right':-60},
    ...
    dbc.Row([
        dbc.Col([
            html.Div([
                html.P("% Increase/Decrease in", style={'text-align':'center','font-family':'arial', 'font-weight':'bold','padding-bottom':0, 'margin-bottom':0}),
        ...
        {
            'if': {
                'filter_query': '{Female Femur} <= -4.7',
                'column_id': 'Female Femur'},
            'backgroundColor': 'dodgerblue',
            'color': 'white',
         },
            {'if': {
                'filter_query': '{Male Femur} <= -5.8',
                'column_id': 'Male Femur'},
            'backgroundColor': 'dodgerblue',
            'color': 'white'
            },
        ...
        dbc.Col([
            dbc.Row([
                        dcc.Graph(id='box_plot2', config={'modeBarButtonsToRemove': ['pan2d', 'displaylogo','autoScale2d','resetScale2d',
            ...
            dbc.Row([
                        dcc.Graph(id='box_plot3', config={'modeBarButtonsToRemove': ['pan2d', 'displaylogo','autoScale2d','resetScale2d',
                        'hoverClosestCartesian','lasso2d','zoomIn2d','zoomOut2d','zoom2d','select2d','toggleSpikelines',
                        'hoverCompareCartesian','toggleHover'],'displayModeBar': True, 'displaylogo': False,
                        'toImageButtonOptions':{'format': 'png'}}) #'displayModeBar': False
    ...
    dbc.Row([
        ...
        dbc.Col([html.P('P values were calculated based on the outcomes of a two sample t-test',
                        style={'color': 'black', 'fontSize': 12,'fontFamily': "Verdana",
                               'margin-top':20, 'textAlign':'center'}),
        ...
        dbc.Col([
            html.Div([
                dbc.Button(
                    ...
                    style=CT,
                    n_clicks=0,
                    href='https://ucsci.uchc.edu/bonebase/GeneEntry.html?GeneSymbol=Irf8+MicroCT_FemurCort',
                    target='_blank',
                    external_link=True
                ),

                    ],style={'margin-left':60}),
            ],width=2, style={'margin-left':50}),

        dbc.Col(html.Div([layout_CORT()], style={'margin-top':0, 'margin-left':-10}),
                width=2, style={'margin-left':-40}),

        ], style={'height': 70, 'padding-bottom': 0, 'padding-top': 0, 'margin-bottom':0,'margin-left':0,'margin-right':-85,}),
...
@callback(
    Output(component_id='box_plot2', component_property='figure'),
    Input(component_id='Table2', component_property='selected_rows')
    )
...
def update_boxplots(selected_rows):
        if len(selected_rows) == 0:
            ...
            y8 = Mask
            y9 = controlfemaleFemur_df['Ct.Mask']
            y10 = Mask2
            y11 = controlmaleFemur_df['Ct.Mask']
            y12 = CtArTtAr
            y13 = controlfemaleFemur_df['Ct.Ar_Tt.Ar']
            y14 = CtArTtAr2
            y15 = controlmaleFemur_df['Ct.Ar_Tt.Ar']
        ...
        box_plot = make_subplots(rows=2, cols=4,
            ...
            subplot_titles=("<b>Ct.Mask </b>(mm\u00b2)", "<b>Ma.Ar </b>(mm\u00b2)","<b>Tt.Ar </b>(mm\u00b2)",'<b>Ct.Ar/Tt.Ar</b> (%)',
                            "<b>Ct.Mask </b>(mm\u00b2)","<b>Ma.Ar </b>(mm\u00b2)","<b>Tt.Ar </b>(mm\u00b2)",'<b>Ct.Ar/Tt.Ar</b> (%)',
                           ))
        ...
        box_plot.update_yaxes(title_text="<b>Female</b>", row=1, col=1,gridcolor='#D3D3D3', title_font=dict(size=18,family='Arial'),tickfont=dict(size=14), range=[0.4,1.0])
        box_plot.update_yaxes(row=2, col=1,title_text="<b>Male</b>",gridcolor='#D3D3D3',title_font=dict(size=18,family='Arial'),tickfont=dict(size=14),range=[0.4,1.0])
...
@callback(
    Output(component_id='box_plot3', component_property='figure'),
    Input(component_id='Table2', component_property='selected_rows')
    )
def update_boxplots(selected_rows):
    if len(selected_rows) == 0:
        ...
        y16 = CtAr
        y17 = controlfemaleFemur_df['Ct.Ar']
        y18 = CtAr2
        y19 = controlmaleFemur_df['Ct.Ar']
        y20 = PsPm
    ...
    box_plot = make_subplots(rows=2, cols=4,
        ...
        # for colspan and rowspan default is 1 col or 1 row so setting up the spect like this
        # defines a 2x4 subplot
        subplot_titles=( "<b>Ct.Ar</b> (mm\u00b2)", "<b>Ps.Pm</b> (mm)","<b>Ec.Pm</b> (mm)", '<b>Ct.Th</b> (mm)',
    ...
    box_plot.update_layout(height=600, width=900, margin={'t': 30},plot_bgcolor='white',font_family='Arial')
    box_plot.update_yaxes(title_text="<b>Female</b>", row=1, col=1,gridcolor='#D3D3D3', title_font=dict(size=18,family='Arial'),tickfont=dict(size=14), range=[0.3,1.0])
    box_plot.update_yaxes(row=2, col=1,title_text="<b>Male</b>",gridcolor='#D3D3D3',title_font=dict(size=18,family='Arial'),tickfont=dict(size=14),range=[0.3,1.0])
...
def display_page_contents(n_clicks,selected_rows):
    ...
    if len(selected_rows) > 0 & n_clicks >= 0:
        ...
        s = value
        n = ''.join([str(elem) for elem in s])
        string = 'https://ucsci.uchc.edu/bonebase/GeneEntry.html?GeneSymbol=Irf8+MicroCT_FemurCort'
        pathname = string.replace('Irf8', n)
        print(pathname)
        return (pathname)
    elif n_clicks==1 & len(selected_rows)== 0:
        return 'https://ucsci.uchc.edu/bonebase/GeneEntry.html?GeneSymbol=Irf8+MicroCT_FemurCort'
