...
import pathlib
dash.register_page(__name__, path="/HOMO_HISTO_FTRAB_OB")
...
FemaleFemur_Count=controlfemaleFemur_df['MAR'].count()
FFlist=controlfemaleFemur_df['MAR'].dropna().to_list()
FF_MARave=(sum(FFlist)/(FemaleFemur_Count))



MaleFemur_Count=controlmaleFemur_df['MAR'].count()
MFlist=controlmaleFemur_df['MAR'].dropna().to_list()
MF_MARave=(sum(MFlist)/(MaleFemur_Count))


ratio_df['Gene Symbol']=data1['gene_symbol']
ratio_df['Female Femur']=((((data1['MAR'])/FF_MARave)-1)*100)
ratio_df['Male Femur']=((((data2['MAR'])/MF_MARave)-1)*100)
...
dbc.Col([
    # row  2
    dbc.Row(
        [html.H1('Femur Osteoblast Histomorphometric Analyses of Global Homozygous KO Mice',
                        style={'color': 'black', 'fontSize': 25,'fontFamily': "Verdana","font-weight":"600",
                               'margin':'auto'}),

        ], style={'height': 60, 'padding-bottom': 0, 'padding-top': 0, 'margin-bottom':0, 'margin-left':20, 'margin-right':-100},
    ...
    dbc.Row([
        dbc.Col([
            ...
            # },
            {'if': {
                'filter_query': '{Female Femur} >= 16.5',
                'column_id': 'Female Femur'},
            'backgroundColor': 'tomato',
            'color': 'white'
            },
            {'if': {
                'filter_query': '{Male Femur} >= 16.0',
        ...
        {
            'if': {
                'filter_query': '{Female Femur} <= -14.0',
                'column_id': 'Female Femur'},
            'backgroundColor': 'dodgerblue',
            'color': 'white',
         },
            {'if': {
                'filter_query': '{Male Femur} <= -12.3',
                'column_id': 'Male Femur'},
            'backgroundColor': 'dodgerblue',
            'color': 'white'
            },
        ...
        dbc.Col([
            dbc.Row([
                        dcc.Graph(id='box_plot6', config={'modeBarButtonsToRemove': ['pan2d', 'displaylogo','autoScale2d','resetScale2d',
            ...
            dbc.Row([
                        dcc.Graph(id='box_plot7', config={'modeBarButtonsToRemove': ['pan2d', 'displaylogo','autoScale2d','resetScale2d',
                        'hoverClosestCartesian','lasso2d','zoomIn2d','zoomOut2d','zoom2d','select2d','toggleSpikelines',
                        'hoverCompareCartesian','toggleHover'],'displayModeBar': True, 'displaylogo': False,
                        'toImageButtonOptions':{'format': 'png'}}) #'displayModeBar': False
    ...
    dbc.Row([
        ...
        ],width=2,style={'margin-left':20, 'margin-top':15}),

        dbc.Col(html.Div([layout_OB()], style={'margin-left': 0}),
                width=2, style={'margin-left':20,'margin-top': 15, 'margin-right': -20}),
...
@callback(
    Output(component_id='box_plot6', component_property='figure'),
    Input(component_id='Table5', component_property='selected_rows')
    )
...
def update_boxplots(selected_rows):
        if len(selected_rows) == 0:
            ...
            y = mar
            y1 = controlfemaleFemur_df['MAR'].dropna()
            y2 = mar2
            y3 = controlmaleFemur_df['MAR'].dropna()
            y4 = msbs
            y5 = controlfemaleFemur_df['MS_BS'].dropna()
            y6 = msbs2
            y7 = controlmaleFemur_df['MS_BS'].dropna()
            y8 = bfr
            y9 = controlfemaleFemur_df['BFR'].dropna()
            y10 = bfr2
            y11 = controlmaleFemur_df['BFR'].dropna()
            y12 = apbs
            y13 = controlfemaleFemur_df['AP_BS'].dropna()
            y14 = apbs2
            y15 = controlmaleFemur_df['AP_BS'].dropna()
...
@callback(
    Output(component_id='box_plot7', component_property='figure'),
    Input(component_id='Table5', component_property='selected_rows')
    )
def update_boxplots(selected_rows):
    if len(selected_rows) == 0:
        ...
        y16 = aplbs
        y17 = controlfemaleFemur_df['AP_L_BS'].dropna()
        y18 = aplbs2
        y19 = controlmaleFemur_df['AP_L_BS'].dropna()
        y20 = apnlbs
        y21 = controlfemaleFemur_df['AP_NL_BS'].dropna()
        y22 = apnlbs2
        y23 = controlmaleFemur_df['AP_NL_BS'].dropna()
        y24 = aplap
        y25 = controlfemaleFemur_df['AP_L_AP'].dropna()
        y26 = aplap2
        y27 = controlmaleFemur_df['AP_L_AP'].dropna()
        y28 = apnlap
        y29 = controlfemaleFemur_df['AP_NL_AP'].dropna()
        y30 = apnlap2
        y31 = controlmaleFemur_df['AP_NL_AP'].dropna()
...
@callback(
    Output('btn-nclicks-5', 'href'),
    Input('btn-nclicks-5','n_clicks'),
    Input('Table5','selected_rows')
)
