...
import pathlib
dash.register_page(__name__, path="/HOMO_HISTO_VTRAB")
...
FemaleVert_Count=controlfemaleHISTOVert_df['BV_TV'].count()
FVlist=controlfemaleHISTOVert_df['BV_TV'].dropna().to_list()
FV_BVTVave=(sum(FVlist)/(FemaleVert_Count))

MaleVert_Count=controlmaleHISTOVert_df['BV_TV'].count()
MVlist=controlmaleHISTOVert_df['BV_TV'].dropna().to_list()
MV_BVTVave=(sum(MVlist)/MaleVert_Count)

ratio_df['Gene Symbol']=data1['gene_symbol']
ratio_df['Female Vertebra']=((((data1['BV_TV'])/FV_BVTVave)-1)*100)
ratio_df['Male Vertebra']=((((data2['BV_TV'])/MV_BVTVave)-1)*100)
...
dbc.Col([
    # row  2
    dbc.Row(
        [html.H1('Vertebral Histological Analysis of Male & Female Global Homozygous KO Mice',
    ...
    dbc.Row([
        dbc.Col([
            html.Div([
                html.P("% Increase/Decrease in BV/TV", style={'font-family':'arial', 'font-weight':'bold', 'padding-left':70, 'padding-bottom':0, 'margin-bottom':0}),
            ...
            {'if': {
                'filter_query': '{Female Vertebra} >= 13.0',
            ...
            # },
            {'if': {
                'filter_query': '{Male Vertebra} >= 13.0',
        ...
        #     },
        {
            ...
            {'if': {
                'filter_query': '{Male Vertebra} <= -14.5',
                'column_id': 'Male Vertebra'},
            'backgroundColor': 'dodgerblue',
            'color': 'white'
            }
        ...
        ],width=4, style={'margin-left':0,'margin-right':0}),

        # dbc.Col([], width=1),
        dbc.Col([
            dcc.Graph(id='box_plot10', config={'modeBarButtonsToRemove': ['pan2d', 'displaylogo','autoScale2d','resetScale2d','hoverClosestCartesian',
    ...
    dbc.Row([
        ...
        dbc.Col([html.P('P values were calculated based on the outcomes of a two sample t-test',
                        style={'color': 'black', 'fontSize': 12,'fontFamily': "Verdana",
                               'margin-top':0, 'margin-left':40, 'margin-right': -30,'textAlign':'center',}),
        ...
        ],width=2, style={'margin-left':30, 'margin-top':5}),

        dbc.Col(html.Div([layout_SB()], style={'margin-top': 5, 'margin-left': 20}),
...
@callback(
    Output(component_id='box_plot10', component_property='figure'),
    Input(component_id='Table7', component_property='selected_rows')
    )
...
@callback(
    Output('btn-nclicks-7', 'href'),
    Input('btn-nclicks-7','n_clicks'),
    Input('Table7','selected_rows')
)
